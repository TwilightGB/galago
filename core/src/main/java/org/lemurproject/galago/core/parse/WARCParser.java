...
public class WARCParser extends DocumentStreamParser {
    ...
    public Document nextDocument() throws IOException {
        ...
        // there can be several fields that hold the doc ID, start with the one in the
        // WARC standard, then look for different fields we've encountered.
        String doc_id = record.getHeaderMetadataItem("WARC-Record-ID");

        if (doc_id == null || doc_id.length() == 0) {
            doc_id = record.getHeaderMetadataItem("WARC-TREC-ID");
        }
        if (doc_id == null || doc_id.length() == 0) {
            // this field is for the ArabicWeb16 collection
            doc_id = record.getHeaderMetadataItem("WARC-DOC-ID");
        }
        Document doc = new Document(doc_id, record.getContentUTF8());
